...
float4 ps_DiffuseMap_BW( float2 Texcoord: TEXCOORD0) : COLOR0
{      
	
	float4 fvBaseColor = tex2D( diffuseMap, Texcoord );

	return (0.1*fvBaseColor.x + 0.95*fvBaseColor.y+0.2*fvBaseColor.z); 
...
technique CHARACTER_PICTURE_SELECTED
{
	pass p0
    {
		PixelShader = compile ps_3_0 ps_DiffuseMap_Selected();
...
technique CHARACTER_PICTURE_DEAD
{
    pass p0
    {
        
			
			PixelShader = compile ps_3_0 ps_DiffuseMap_BW();
    }
}